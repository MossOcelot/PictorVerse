...
public class Bank_Manager : MonoBehaviour
{
    ...
    [SerializeField]
    private AccountData.Accounts player_account;
    [SerializeField]
    ...
    public AccountData.Accounts GetPlayer_Account()
    {
    ...
    private void Update()
    {
        if (!GetPlayerAccount())
        {
            AccountData.Accounts newAccount = new AccountData.Accounts("xxx",player_status.player_id, "00", 0, section_name.ToString(), section_name.ToString(), new List<AccountList>());
    ...
    {
        AccountData.Accounts account = DB.GetAccount(player_account.account_id);
        if (account != null) {
