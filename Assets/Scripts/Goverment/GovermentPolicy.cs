        ...
        if (date[2] == govermentPolicy.taxCollectionDay[2])
        {
            if (date[1] == govermentPolicy.taxCollectionDay[1])
            {
                if (date[0] == govermentPolicy.taxCollectionDay[0])
                {
                    ...
                    mail_manager.AddMail("Goverment", $"ถึงเวลาจ่ายภาษีแล้ว ",
                        $"โปรดจ่ายภาษีของปี {date[2] - 1} โปรดยื่นภาษีและจ่ายภาษีให้เรียบร้อยภายใน 30 วัน นับจากวันที่ {date[0]}/{date[1]}/{date[2]}", null, null);
